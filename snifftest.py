...
def clean(ip):
    ...
    while g and g[0][0] < now - TIME_LIMIT:
        g.popleft()
...
def report(ip,port,attempts):
        ...
        if now - last_port_alert[ip]> COOLDOWN_TIME:
            ...
            if port_count > PORT_LIMIT:
                ...
                while connections[ip]:
                    connections[ip].popleft()
        ...
        if now - last_udp_alert[ip] > COOLDOWN_TIME:
            ...
            if packet_count > PACKET_LIMIT:
                print(f"ALERT: High UDP packet rate from IP: {ip} ({packet_count} unique UDP ports POSSIBLE UDP SCAN!)")
...
def analyze(packet):
    ...
    if port != SSH_PORT:
        ...
        ssh_attempts[src_ip].append(time.time())
        print(f"DEBUG: New SSH SYN from {src_ip}, total attempts: {len(ssh_attempts[src_ip])}")
